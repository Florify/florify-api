...
class FlorifyDataLoader {
  ...
  getSinglePlant(id, timePeriod) {
    ...
    .then(result => {
      ...
      return this.query(
        `
        select type, AVG(reading) as reading,
        FROM_UNIXTIME(FLOOR(unix_timestamp(createdAt)/?)*?) as startTime
        FROM data where plantId = ? AND createdAt >= DATE_SUB(NOW(), INTERVAL ? HOUR)
        GROUP BY type, startTime;
        `,
        [timeConstant, timeConstant, plant.id, timeIntervalInHours]
      )
    })
    .then( datapoints => {
      ...
      averagedData.timeAxis = averagedData.hum.map(obj => obj.startTime);
      averagedData.hum = averagedData.hum.map(obj => obj.reading);
      averagedData.lux = averagedData.lux.map(obj => obj.reading);
      averagedData.ph = averagedData.ph.map(obj => obj.reading);
      averagedData.temp = averagedData.temp.map(obj => obj.reading);
      return averagedData;
    })
    .catch(console.error)
  }
