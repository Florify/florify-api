...
class FlorifyDataLoader {
  ...
  getSinglePlant(id, type, time) {
    if (time === 'day'){
      time = moment().substract(1, 'days')
    }
    if (time === 'week'){
      time = moment().substract(1, 'weeks')
    }
    if (time === 'month'){
      time = moment().substract(1, 'months')
    }
    let plant = {};
    return this.query(
      knex
    ...
    .then(result =>{
      // console.log(result, "hallloooooo");
      plant = result[0];
      plant.readings = {};
      // console.log('plantid', id);
      // console.log(plant, "ooha")
      ...
      .then(datapoints => {
        ...
        plant.readings.hum = datapoints.filter((point)=>{
          if (point.type === 'hum'){
            return point;
          }
        })
