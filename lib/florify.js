...
class FlorifyDataLoader {
  ...
  getSinglePlant(id, timePeriod) {
    ...
    .then(datapoints => {
      ...
      let averagedData = timeArray.reduce((acc, time, index) => {

        let formattedTime = moment(time).format('X')
        let hum = [];
        let temp = [];
        let ph = [];
        let lux = [];
        let j=0;
        while(datapoints[j] && moment(datapoints[j].createdAt).format('X') < formattedTime) {
          if (datapoints[j].type === 'temp') temp.push(datapoints[j].reading)
          else if (datapoints[j].type === 'lux') lux.push(datapoints[j].reading)
          else if (datapoints[j].type === 'hum') hum.push(datapoints[j].reading)
          else if (datapoints[j].type === 'ph') ph.push(datapoints[j].reading)
          j++;
        }
        datapoints.splice(0,j)
